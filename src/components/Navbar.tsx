...
const Navbar: React.FC = () => {
  ...
  const NavLinkComponent: React.FC<{ item: NavLinkItem }> = ({ item }) => {
    if (item.isScroll && isHomePage) {
      return (
        <ScrollLink
          ...
          className={`relative font-bold text-gray-800 hover:text-blue-600 transition-all duration-300 cursor-pointer group text-center text-base md:text-lg font-assistant ${isRTL ? 'text-right' : 'text-left'}`}
        >
    ...
    return (
      <Link to={item.to} onClick={() => setIsMenuOpen(false)} className={`relative font-bold text-gray-800 hover:text-blue-600 transition-all duration-300 group text-center text-base md:text-lg font-assistant ${isRTL ? 'text-right' : 'text-left'}`}>
        <span className={`transition-opacity duration-200 ${isTransitioning ? 'opacity-60' : 'opacity-100'}`}>{item.label}</span>
        <span className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 transition-all duration-300 group-hover:w-full"></span>
      </Link>
    );
  ...
  return (
    <>
      <header className={`sticky top-0 z-50 transition-transform duration-300 ${isRTL ? 'rtl' : 'ltr'} ${isNavbarVisible ? 'translate-y-0' : '-translate-y-full'} lg:p-2 lg:p-4 font-assistant`}>
        <div className="container mx-auto px-4 lg:px-3 lg:px-4 lg:rounded-2xl lg:rounded-full lg:bg-white/60 lg:backdrop-blur-xl lg:border lg:border-white/30 lg:shadow-lg transition-all duration-300 bg-white border-b border-gray-200 font-assistant">
          <nav className="relative flex items-center justify-between h-16">
            {/* Logo - Always on the left for LTR, right for RTL */}
            <div className={`flex items-center ${isRTL ? 'order-3' : 'order-1'}`}>
              <Link to="/" className={`flex-shrink-0 flex items-center ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'} group font-assistant`}>
                <img src={logo} alt="Blabber Logo" className="h-8 w-auto" />
              </Link>
            </div>

            {/* Centered Nav Links for Desktop */}
            <div className={`hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 ${isRTL ? 'order-2' : 'order-2'}`}>
              <div className={`flex items-center ${isRTL ? 'space-x-reverse space-x-8' : 'space-x-8'} font-assistant`}>
            ...
            <div className={`flex items-center ${isRTL ? 'order-1' : 'order-3'}`}>
              <div className={`hidden lg:flex items-center ${isRTL ? 'space-x-reverse space-x-4' : 'space-x-4'} font-assistant`}>
                <CtaButton />
                <div className="relative flex items-center bg-gray-100/70 rounded-full p-1 border border-gray-200/50 transition-all duration-300 hover:border-blue-300 hover:bg-blue-50 w-20 xl:w-24 justify-center font-assistant">
      ...
      >
        <div className="container mx-auto px-4 h-full flex flex-col">
          ...
          <div className={`flex-grow flex flex-col items-center justify-center space-y-8 ${isRTL ? 'text-right' : 'text-center'}`}>
            {navLinks.map((item) => (
