...
const Navbar: React.FC = () => {
  ...
  const NavLinkComponent: React.FC<{ item: NavLinkItem }> = ({ item }) => {
    if (item.isScroll && isHomePage) {
      return (
        <ScrollLink
          ...
          className="relative font-bold text-gray-800 hover:text-blue-600 transition-all duration-300 cursor-pointer group text-center text-base md:text-lg font-assistant"
        >
    ...
    return (
      <Link to={item.to} onClick={() => setIsMenuOpen(false)} className="relative font-bold text-gray-800 hover:text-blue-600 transition-all duration-300 group text-center text-base md:text-lg font-assistant">
        <span className={`transition-opacity duration-200 ${isTransitioning ? 'opacity-60' : 'opacity-100'}`}>{item.label}</span>
        <span className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 transition-all duration-300 group-hover:w-full"></span>
      </Link>
    );
        ...
        <div className="container mx-auto px-4 lg:px-3 lg:px-4 lg:rounded-2xl lg:rounded-full lg:bg-white/60 lg:backdrop-blur-xl lg:border lg:border-white/30 lg:shadow-lg transition-all duration-300 bg-white border-b border-gray-200 font-assistant">
          <nav className="relative flex items-center justify-between h-16">
            ...
            <div className="flex items-center">
              <Link to="/" className="flex-shrink-0 flex items-center space-x-2 group font-assistant">
            ...
            <div className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              <div className="flex items-center space-x-8 font-assistant">
            ...
            {/* Right Side */}
            <div className="flex items-center">
          <div className="hidden lg:flex items-center space-x-4 font-assistant">
                <CtaButton />
